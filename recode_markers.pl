...
foreach(@combining_table){
	...
	print "Alleles to combine ".$data[1]." data $data[2] here\n";
	...
	print "OUTPUTTING RECODED MARKER $new_allele_name  $data[3]\n";
	my $outpath=$pathtooutput.$name."/";
	output_marker(\@combined_marker, $outpath, $data[3]);
#	exit;   
...
print $name."\n";
print "UNDER\n";
my $loc_name=($pathtooutput.$name); 
$loc_name=~s/\r//g;
$loc_name=$loc_name.".loc";

print "OUTPUTTING TO $loc_name\n";
  open(OUT,">$loc_name");
...
# "ABCN\n";
my %code3=("lm1"=>"a","lm2"=>"b","np1"=>"c","nn1"=>"d","--1"=>'--',"--2"=>'--',"ll1"=>"","ll2"=>"","nn2"=>"");
# "ANCN\n
my %code4=("lm1"=>"a","ll1"=>"b","np1"=>"c","nn1"=>"d","--1"=>'-',"--2"=>'-');
...
print "lm $lm np $np \n";
	...
	print "ANCN\n";
	@recode=codeabcd(\@AoA,\%code4,\$length);
	unshift(@recode,'<abxcd>')
	#print $length;
	}
return (@recode);
